...
@customElement(SOLAR_CHART_CARD_EDITOR_NAME)
export class AdaptiveCoverProSolarChartCardEditor extends LitElement implements LovelaceCardEditor {
  ...
  private _emit(next: SolarChartCardConfig): void {
    this._config = next;
    this.dispatchEvent(
  ...
  private _baseConfig(): SolarChartCardConfig {
    return this._config ?? { type: `custom:${SOLAR_CHART_CARD_NAME}`, entry_ids: [] };
  }
  ...
  private _emitWithColors(
    base: SolarChartCardConfig,
    colors: (string | null)[],
    overrides?: Partial<SolarChartCardConfig>,
  ): void {
    const trimmed = this._trimColors(colors);
    const { cover_colors: _cc, ...rest } = base;
    void _cc;
    const next: SolarChartCardConfig = trimmed
      ? { ...(rest as SolarChartCardConfig), ...overrides, cover_colors: trimmed }
      : { ...(rest as SolarChartCardConfig), ...overrides };
    this._emit(next);
  }
  ...
  private _onTitleChange(e: Event): void {
    ...
    if (value) this._emit({ ...base, title: value });
      ...
      void _title;
      this._emit(rest as SolarChartCardConfig);
    }
  }
  ...
  protected render(): TemplateResult | typeof nothing {
    ...
    return html`
      <div class="form">
        <div class="section">
          <label class="field-label">${t('editor.solar_chart.instances', this.hass)}</label>
          <div class="hint">${t('editor.solar_chart.instances_hint', this.hass)}</div>
          ...
          ? html`
              <div class="section">
                <label class="field-label"
                  >${t('editor.solar_chart.cover_colors', this.hass)}</label
                >
                <div class="hint">${t('editor.solar_chart.cover_colors_hint', this.hass)}</div>
                    ...
                    <div class="color-row">
                      ...
                      <span class="toggle-text">
                        ...
                        <span class="toggle-desc"
                          >${override
                            ? override
                            : t('editor.solar_chart.default_color', this.hass)}</span
                        >
                      </span>
                      <button
                        type="button"
        ...
        <div class="section">
          <label class="field-label">${t('editor.solar_chart.display', this.hass)}</label>
          <label class="toggle-row">
            <input
              type="checkbox"
              .checked=${this._config.compact ?? false}
              @change=${(e: Event) =>
                this._onToggle('compact', (e.target as HTMLInputElement).checked)}
            />
            <span class="toggle-text">
              <span class="toggle-label"
                >${t('editor.solar_chart.toggle_compact_label', this.hass)}</span
              >
              <span class="toggle-desc"
